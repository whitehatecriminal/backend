...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const user = await User.create({
    //Creating entry of a user in Database
    fullname,
    avatar: avatar.url,
  ...
  const createduser = await User.findByPk(user.id, {
    //remove password or change password mode
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, {}, "Account details updated"));
});
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, updatedavatar, "avatar  updated susscefully"));
});
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, updatedcoverImage, "coverImage  updated susscefully")
    );
});
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const isSubscribed = channel.subscribers.some(
    (sub) => sub.subscriber.toString() === req.user?.id.toString()
  );
